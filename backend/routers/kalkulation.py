...
class CalcCreate(BaseModel):
    calc_name: str
    machine: str
    material_group: str   # AlSi10Mg, 1.4404, IN718_IN625
...
class AddPartsRequest(BaseModel):
    part_ids: List[int]           # individual part IDs
    inquiry_id: Optional[int] = None  # OR entire inquiry
...
def serialize_calc(calc: CombinedCalculation, db: Session) -> dict:
    ...
    for cp in calc.calc_parts:
        ...
        parts_data.append({
            ...
            "quantity":                 qty,
            "part_volume_mm3":          float(part.part_volume_mm3) if part.part_volume_mm3 else None,
            "stock_cm3":                float(part.stock_cm3) if part.stock_cm3 else None,
            "support_volume_cm3":       float(part.support_volume_cm3) if part.support_volume_cm3 else None,
            "part_height_mm":           float(part.part_height_mm) if part.part_height_mm else None,
            "prep_time_min":            float(part.prep_time_min) if part.prep_time_min else None,
            "post_handling_time_min":   float(part.post_handling_time_min) if part.post_handling_time_min else None,
            "blasting_time_min":        float(part.blasting_time_min) if part.blasting_time_min else None,
            "leak_testing_time_min":    float(part.leak_testing_time_min) if part.leak_testing_time_min else None,
            "qc_time_min":              float(part.qc_time_min) if part.qc_time_min else None,
            ...
            "manual_build_time_h":      float(part.manual_build_time_h) if part.manual_build_time_h else None,
...
def add_parts(
    ...
    for part_id in part_ids:
        ...
        if mat_group != calc.material_group and mat != calc.material_group:
            continue

        # Check if already added
        existing = db.query(CalcPart).filter(
            CalcPart.calc_id == calc_id,
            CalcPart.part_id == part_id
        ).first()
        if existing:
            continue

        # Run regression
        pred = predict(calc.machine, mat, {
    ...
    db.commit()
    return {"message": f"{added} Bauteile zur Kalkulation hinzugefügt"}
...
def update_calc_part(
    ...
    # Re-run regression with overrides
    material = cp.material_override or part.material
    quantity = cp.quantity_override or part.quantity
    pred = predict(calc.machine, material, {
        "quantity":              quantity,
        "part_volume_mm3":       float(part.part_volume_mm3 or 0),
        "stock_cm3":             float(part.stock_cm3 or 0),
        "support_volume_cm3":    float(part.support_volume_cm3 or 0),
    ...
    manual_price = float(part.manual_part_price_eur or 0) * quantity
    calc_price = float(cp.calc_part_price_eur or 0) * quantity if cp.calc_part_price_eur else None
    cp.price_reduction_eur = round(manual_price - calc_price, 2) if calc_price else None
    cp.price_reduction_percent = round((manual_price - calc_price) / manual_price * 100, 1) if calc_price and manual_price else None
...
def get_available_parts(
    ...
    """Get all parts from Datenbank compatible with given machine and material group."""
    inquiries = db.query(Inquiry).filter(Inquiry.machine == machine).all()
