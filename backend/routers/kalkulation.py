...
def get_available_parts(
    ...
    """Get ALL parts from Datenbank for given machine+material group.
    Parts are always available regardless of whether they were used in (deleted) calculations."""
...
def update_calc_part(
    ...
    for field, value in data.dict(exclude_unset=True).items():
        override_field = field if field in ("material_override", "quantity_override") else f"{field}_override"
